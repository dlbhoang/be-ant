...
exports.createOrder = async (req, res, next) => {
  try {
    ...
    if (courses.length == 0) {
      return next(new ErrorResponse('Không tìm thấy khóa học nào', 404));
    }
    ...
    for (const courseId of courses) {
      ...
      if (course.studentsEnrolled.includes(user._id)) {
        return next(new ErrorResponse('Sinh viên đã được ghi danh vào một khóa học', 404));
      }
    ...
    next(new ErrorResponse('Không tạo được đơn hàng. Vui lòng thử lại', 500));
  }
};
...
exports.verifyPaymentSignature = async (req, res, next) => {
  try {
    ...
    if (generated_signature !== razorpay_signature) {
      return next(new ErrorResponse('Yêu cầu không hợp lệ', 400));
    }
    ...
    return res.status(200).json({
      success: true,
      data: 'Thanh toán đã được xác minh và sinh viên đã đăng ký khóa học',
    });
  } catch (err) {
    next(new ErrorResponse('Không thể xác minh chữ ký', 500));
  }
};
...
exports.sendPaymentSuccessEmail = async (req, res, next) => {
  try {
    ...
    if (!(orderId && paymentId && amount)) {
      return next(new ErrorResponse('Không gửi được email thanh toán thành công, thiếu một số trường', 404));
    }

    const user = await User.findById(req.user.id);
    const emailResponse = await emailSender(user.email, 'Đã nhận được thanh toán', paymentSuccessEmailTemplate(`${user.firstName} ${user.lastName}`, amount / 100, orderId, paymentId));

    res.status(200).json({
      success: true,
      data: 'Email thanh toán thành công đã được gửi thành công',
    });
  } catch (error) {
    next(new ErrorResponse('Không gửi được email thanh toán thành công', 500));
  }
};
